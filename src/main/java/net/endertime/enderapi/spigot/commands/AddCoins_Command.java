...
public class AddCoins_Command implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if ((sender instanceof Player)) {
            ...
            if (p.hasPermission("coinapi.addcoins")) {
                if (args.length == 2) {
                    ...
                    if (target != null) {
                        ...
                        p.sendMessage(EnderAPI.getInstance().getPrefix() + "§7Du hast dem Spieler §c" + target.getName()
                                + " §6" + amount + " §5EnderCoins §2hinzugefügt");
                    } else {
                        p.sendMessage(EnderAPI.getInstance().getPrefix() + "§7Der Spieler §c" + args[0]
                                + " §7ist §cnicht §7online!");
                    }
                } else {
                    p.sendMessage(EnderAPI.getInstance().getPrefix() + "§8/§7addcoins §8<§7Name§8> <§7Anzahl§8>");
                }
            } else {
            ...
            if (target != null) {
                ...
                sender.sendMessage(EnderAPI.getInstance().getConsolePrefix() + "§7Du hast dem Spieler §c"
                        + target.getName() + " §6" + amount + " §5EnderCoins §2hinzugefügt");
            } else {
                sender.sendMessage(EnderAPI.getInstance().getConsolePrefix() + "§7Der Spieler §c" + args[0]
                        + " §7ist §cnicht §7online!");
            }
        } else {
            sender.sendMessage(EnderAPI.getInstance().getConsolePrefix() + "§8/§7addcoins §8<§7Name§8> <§7Anzahl§8>");
        }
        return false;
    }
}
